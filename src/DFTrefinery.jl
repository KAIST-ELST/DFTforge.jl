...
function Qspace_Ksum_atomlist_parallel_nc(kq_function,q_point_list,k_point_list,
  atom12_list::Vector{Tuple{Int64,Int64}},theta_phi_list,num_return=1,
  result_index=1,cache_index=1)
  batch_size = 2*nprocs();
  progress_step = 10;
  if (length(q_point_list) < 50)
      progress_step = 2;
  end
  if (20 > batch_size)
    batch_size = 20;
  ...
  Xij_Q = Array(Dict{k_point_int_Tuple,Array{Complex_my,1}},num_return,length(theta_phi_list),length(atom12_list));
  Xij_Q_mean = Array(Dict{k_point_int_Tuple,Complex_my},num_return,length(theta_phi_list),length(atom12_list));

  for xyz_ij = 1:num_return
    for theta_phi_i = 1:length(theta_phi_list)
        for atom12_i = 1:length(atom12_list)
            Xij_Q[xyz_ij,theta_phi_i,atom12_i] = Dict{k_point_int_Tuple,Array{Complex_my,1}}();
            Xij_Q_mean[xyz_ij,theta_phi_i,atom12_i] = Dict{k_point_int_Tuple,Complex_my}();
        end
    end
  end
  #Q_ksum = Dict{k_point_int_Tuple,Array{Complex_my,1}}();
    ...
    for xyz_ij = 1:num_return
      for theta_phi_i = 1:length(theta_phi_list)
        for atom12_i = 1:length(atom12_list)
          tmp = zeros(Complex_my,length(k_point_list))
          for ii = 1:length(k_point_list)
              tmp[ii] = X_temp[ii][xyz_ij,theta_phi_i,atom12_i];
          end
          Xij_Q[xyz_ij,theta_phi_i,atom12_i][q_point_int] = copy(tmp);
          Xij_Q_mean[xyz_ij,theta_phi_i,atom12_i][q_point_int] = mean(tmp);
        end
      end
    end
